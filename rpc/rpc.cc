		...
		unsigned int xid_rep, char **b, int *sz)
{
	...
	// 类比tcp的滑动窗口机制
	if (reply_window_[clt_nonce].front().xid > xid) {	// 如果滑动窗口的第一个值大于xid，表示之前的窗口已经移动了
		return FORGOTTEN;					// 窗口移动，之前的xid被删除，返回FORGOTTEN
	}
	...
	// new xid never seen before
	reply_t reply(xid);
	iter = reply_window_[clt_nonce].begin();
	while (iter != reply_window_[clt_nonce].end()) {
		if (iter->xid > xid) {
